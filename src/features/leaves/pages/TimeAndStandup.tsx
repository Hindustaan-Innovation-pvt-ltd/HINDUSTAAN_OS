...
export default function TimeAndStandup({ session }: { session?: any }) {
  ...
  const role = session?.user?.user_metadata?.role || ctxUser?.role || 'intern';
  ...
  const handleStandupSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const targetUserId = ctxUser?.id || session?.user?.id;
    if (!targetUserId) {
      toast.error("User ID not found. Cannot submit standup.");
      return;
    }
    try {

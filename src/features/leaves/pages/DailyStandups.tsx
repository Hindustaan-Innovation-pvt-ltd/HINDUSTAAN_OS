...
export default function DailyStandups({ session }: { session?: any }) {
  ...
  useEffect(() => {
    const fetchStandups = async () => {
      try {
        ...
        const targetUserId = ctxUser?.id || session?.user?.id || user?.id || (user as any)?.userId;
        if (!targetUserId) return;
        const effectiveRole = ctxUser?.role || user?.role || 'employee';
        if (effectiveRole === 'manager' || effectiveRole === 'admin') {
          ...
          const res = await api.get(`/standups?userId=${targetUserId}&limit=50`);
          if (res.data?.success) {
            const mapped = res.data.data.map((s: any) => ({
              id: s.id,
              user: user?.name || 'Intern',
              initials: (user?.name || 'IN').split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2),
  ...
  const handleUpdateSubmit = async () => {
    ...
    // Submit to backend
    const targetUserId = ctxUser?.id || user?.id || (user as any)?.userId || session?.user?.id;
    if (targetUserId) {
      try {
        ...
        if (res.data?.success) {
          toast.success('Standup saved to database!');
        }
      } catch (err: any) {
        console.warn('Standup save to backend failed:', err.response?.data?.message || err.message);
        toast.error('Could not save standup to database: ' + (err.response?.data?.message || err.message));
        return;
      }
    } else {
      toast.error('User session ID not found. Standup could not be saved to DB.');
      return;
    }
  ...
  // Quick Notes State
  const noteKey = `hindustaan_notes_${currentUserName || 'default'}`;
  const [note, setNote] = useState(() => {
    return localStorage.getItem(noteKey) || localStorage.getItem('hindustaan_quick_notes') || '';
  });

  useEffect(() => {
    const loadNote = () => {
      const saved = localStorage.getItem(noteKey) || localStorage.getItem('hindustaan_quick_notes') || '';
      setNote(saved);
    };
    loadNote();

    const handleStorageUpdate = () => loadNote();
  ...
  const handleNoteChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    localStorage.setItem('hindustaan_quick_notes', value);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (targetUserId) {
      try {
        ...
        toast.success('Standup Submitted!', { description: 'Your standup has been saved to the database.' });
      } catch (err: any) {
        console.warn('Standup save to backend failed:', err.response?.data?.message || err.message);
        toast.error('Standup save to database failed:', { description: err.response?.data?.message || err.message });
      }
    } else {
      toast.error('User ID not found. Standup could not be saved to DB.');
    }

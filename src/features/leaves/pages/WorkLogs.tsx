...
export default function WorkLogs({ session }: { session?: any }) {
  ...
  useEffect(() => {
    if (currentUser.role !== 'employee' && currentUser.role !== 'intern') return;
  ...
  useEffect(() => {
    const updateSessions = () => {
      ...
      teamMembers.forEach(member => {
        const isCurrentUserEmployee = member.id === currentUser.id && (currentUser.role === 'employee' || currentUser.role === 'intern');
  ...
  const isDateMatch = (logDateVal: any, targetDateStr: string) => {
    ...
    const rawStr = String(logDateVal).slice(0, 10);
    return localStr === targetDateStr || utcStr === targetDateStr || rawStr === targetDateStr;
  };
  ...
  const userBaseLogs = useMemo(() => {
    ...
    return base.filter((log: any) => {
      ...
      if (dateFilter) {
        return isDateMatch(log.rawDate || log.date, format(dateFilter, 'yyyy-MM-dd'));
      }
      return true;
    });
  }, [logs, dateFilter]);
  ...
  const uniqueProjects = useMemo(() => {
    const namesFromProjects = (contextProjects || []).map((p: any) => p.name || p.title).filter(Boolean);
    const namesFromLogs = logs.map(l => l.project).filter(Boolean);
    return Array.from(new Set([...namesFromProjects, ...namesFromLogs])).sort();
  }, [logs, contextProjects]);

  const uniqueEmployees = useMemo(() => {
    const namesFromTeam = (teamMembers || []).map((m: any) => m.name).filter(Boolean);
    const namesFromLogs = logs.map(l => l.name).filter(Boolean);
    return Array.from(new Set([...namesFromTeam, ...namesFromLogs])).sort();
  }, [logs, teamMembers]);

  const filteredLogs = useMemo(() => {
    return userBaseLogs.filter((log: any) => {
      // Tab filter
      if (activeTab === 'today' && !isDateMatch(log.rawDate || log.date, todayStr)) return false;
      if (activeTab === 'specific' && !isDateMatch(log.rawDate || log.date, selectedSpecificDate)) return false;
  ...
  }, [userBaseLogs, searchQuery, statusFilter, projectFilter, employeeFilter, activeTab, selectedSpecificDate, currentUser.role, todayStr]);

        ...
        <div className="mt-8">
          ...
          <TabsContent value="requests">
            <div className="flex flex-wrap gap-2 mb-6">
              <Button 
                variant={managerRequestsTab === 'pending' ? 'default' : 'outline'} 
                onClick={() => setManagerRequestsTab('pending')}
                className="rounded-xl font-bold transition-all"
              >
                Pending Requests ({leaveData.filter((l: any) => l.status === 'Pending').length})
              </Button>
              <Button 
                variant={managerRequestsTab === 'processed' ? 'default' : 'outline'} 
                onClick={() => setManagerRequestsTab('processed')}
                className="rounded-xl font-bold transition-all"
              >
